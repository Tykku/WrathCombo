...
internal class BLM_Toshi_Fire4 : CustomCombo
{
    ...
    protected override uint Invoke(uint actionID)
    {
        ...
        return actionID switch
        {
            Fire4 when IsEnabled(Preset.BLM_Toshi_Thunder) && !IsInIcePhase && UseThunder(ThunderHPThreshold(), BLM_ST_ThunderRefresh) => OriginalHook(Thunder),
    ...
    internal class BLM_ToshiXeno : CustomCombo
    {
        protected internal override Preset Preset => Preset.BLM_Toshi_Xeno;
        protected override uint Invoke(uint actionID)
        {
            if (actionID is not Xenoglossy)
                return actionID;
