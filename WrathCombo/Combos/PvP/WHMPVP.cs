        ...
        internal static void Draw(Preset preset)
        {
            switch (preset)
            {
                ...
                case Preset.WHMPvP_Heals:
                    DrawHorizontalMultiChoice(WHMPvP_Heals_Options, "Retarget Cure 2",
                        "To the Heal Stack (In Wrath Settings)", 3, 0);
                    DrawHorizontalMultiChoice(WHMPvP_Heals_Options, "Retarget Cure 3",
                        "To the Heal Stack (In Wrath Settings)", 3, 1);
                    DrawHorizontalMultiChoice(WHMPvP_Heals_Options, "Retarget Aquaveil",
                        "To the Heal Stack (In Wrath Settings)", 3, 2);
                    break;

                case Preset.WHMPvP_Burst_Heals:
                    DrawAdditionalBoolChoice(WHMPvP_Burst_HealsRetarget, "Retarget",
                        "Retargets  to the Heal Stack(In Wrath Settings)");
    ...
    internal class WHMPvP_Burst : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            if (!PvPCommon.TargetImmuneToDamage())
            {
                ...
                if (IsEnabled(Preset.WHMPvP_AfflatusPurgation) && IsLB1Ready &&
                    GetTargetHPPercent() <= WHMPvP_PurgationThreshold)
                ...
                if (CanWeave())
                {
                    ...
                    if (IsEnabled(Preset.WHMPvP_Mirace_of_Nature) && IsOffCooldown(MiracleOfNature) &&
                        InActionRange(MiracleOfNature))
            ...
            {
                IGameObject? healTarget = WHMPvP_Burst_HealsRetarget
                    ? SimpleTarget.Stack.AllyToHealPVP
                    : SimpleTarget.Stack.Allies;

                if (WHMPvP_Burst_Heals_Options[1] && HasStatusEffect(Buffs.Cure3Ready) &&
                    GetTargetHPPercent(healTarget) <= WHMPvP_Burst_HealsThreshold)
                    return WHMPvP_Burst_HealsRetarget
                        ? Cure3.Retarget(Glare, healTarget, true)
                        : Cure3;

                if (WHMPvP_Burst_Heals_Options[2] && ActionReady(Aquaveil) && CanWeave() &&
                    GetTargetHPPercent(healTarget) <= WHMPvP_Burst_HealsThreshold)
                    return WHMPvP_Burst_HealsRetarget
                        ? Aquaveil.Retarget(Glare, healTarget, true)
                        : Aquaveil;

                if (WHMPvP_Burst_Heals_Options[0] && ActionReady(Cure2) &&
                    GetTargetHPPercent(healTarget) <= WHMPvP_Burst_HealsThreshold)
                    return WHMPvP_Burst_HealsRetarget
